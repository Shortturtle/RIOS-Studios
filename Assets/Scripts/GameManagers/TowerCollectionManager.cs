...
public class TowerCollectionManager : MonoBehaviour
{
    ...
    private TowerRewardManager towerRewards;

    private void Start()
    {
        towerRewards = FindFirstObjectByType<TowerRewardManager>();


        if (towerRewards.towerCollectionList[7] == 1) { tower_Portal.SetActive(true); }
        else { tower_Portal.SetActive(false); }

        if(towerRewards.towerCollectionList[1] == 1) { tower_Jinb.SetActive(true); }
        else { tower_Jinb.SetActive(false); }

        if(towerRewards.towerCollectionList[2] == 1) { tower_Potion.SetActive(true); }
        else { tower_Potion.SetActive(false); }

        if(towerRewards.towerCollectionList[3] == 1) { tower_Net.SetActive(true); }
        else { tower_Net.SetActive(false); }

        if(towerRewards.towerCollectionList[4] == 1) { tower_Diver.SetActive(true); }
        else { tower_Diver.SetActive(false); }

        if(towerRewards.towerCollectionList[5] == 1) { tower_Axe.SetActive(true); }
        else { tower_Axe.SetActive(false); }

        if(towerRewards.towerCollectionList[6] == 1) { tower_Railgun.SetActive(true); }
